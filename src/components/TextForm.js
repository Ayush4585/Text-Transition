...
export default function TextForm(props) {
    ...
    return (
        ...
        <h1 className='mb-4 ' style={{color:props.mode==='light'?"#001b53":'white'}}>{props.heading}</h1>
            ...
            <div className="mb-3"> 
            <textarea className="form-control"  placeholder='Type or Paste your text here :)' value={text} onChange={handleOnChange} style={{backgroundColor: props.mode==='dark'?'#13466e':'#d4ffff', color: props.mode==='dark'?'white':'#042743'}} id="myBox" rows="8">{text}</textarea>
            </div>
            ...
            <button disabled={text.length===0} className="btn btn-primary mx-1 my-1" onClick={handleCopy}>Copy Text</button>
            <button disabled={text.length===0} className="btn btn-primary mx-1 my-1" onClick={handlePreview}>Preview</button>
            </div>
            {show && <div className="container my-3" style={{color:props.mode==='dark'?'white':'#042743'}}>
